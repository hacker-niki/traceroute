...
void Traceroute::run(const std::string &host, int max_hops, int timeout, int start_ttl, int port_num) {
    std::string ip = Traceroute::ip_from_name(host);

    std::cout << "Traceroute to " << "\033[1;33m" << ip << "\033[0m for port" << port_num << "\033[0m" << std::endl;
    ...
    socket_address.sin_port = port_num;
...
//        icmp_packet.meta.echo.payload = 0b1011010101101001010101010101010100101010101; // random binary data
        ...
        std::cout << ttl << " " << name_from_ip(inet_ntoa(src_addr.sin_addr)) << " " << "\033[1;35m"
                  << inet_ntoa(src_addr.sin_addr) << "\033[0m " << duration.count() << "ms"
                  << std::endl;

        if (strcmp(inet_ntoa(src_addr.sin_addr), ip.c_str()) == 0) {
            std::cout << std::endl << "\033[1;35m" << ttl << "\033[0m" << " hops between you and " << ip << " "
                      << duration.count()
                      << "ms" << std::endl;
            break;
        }
